...
- Conversation branching is adapter-gated by `messages.read`. ChatGPT and Gemini first attempt a uniquely identifiable, visible native branch command; the extension records no duplicate local branch when that succeeds. If native branching is unavailable, the context-branch builder accepts only visible system/user/assistant plain text through the selected message, bounds long contexts, sanitizes the source URL, and excludes tool messages, attachments, HTML, and hidden reasoning. Cross-tab handoffs live only in `storage.session` (with a background-memory fallback), expire after 15 minutes, fill only through the normal composer adapter, and never activate Send.
