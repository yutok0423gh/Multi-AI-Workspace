...
- Conversation branching on Claude, DeepSeek, Grok, and Kimi opens a clearly labeled context branch and transfers bounded visible system/user/assistant text through an expiring session-only handoff. ChatGPT and Gemini use their own native branching and receive no extension-owned branch button, navigator, or shortcut. Attachments, tools, HTML, and hidden reasoning are excluded; transferred context is never sent automatically or persisted to IndexedDB.
