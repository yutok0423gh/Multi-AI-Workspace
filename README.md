...
- Prompt Manager CRUD, search, tags, cursor-position Markdown/text insertion, JSON import/merge/export, usage counts, and user-gesture variables.
...
- Conversation branching on Claude, DeepSeek, Grok, and Kimi previews the exact bounded system/user/assistant context before opening a clearly labeled context branch. Short context can be filled through an expiring session-only composer handoff; long context is downloaded locally as Markdown and the new chat receives only a short attachment instruction. ChatGPT and Gemini use their own native branching and receive no extension-owned branch button, navigator, or shortcut. Attachments, tools, HTML, and hidden reasoning are excluded; no branch content is sent automatically or persisted to IndexedDB.
